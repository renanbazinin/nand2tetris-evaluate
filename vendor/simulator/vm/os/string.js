...
export function intToCharArray(value) {
    return value
        .toString()
        .split("")
...
export class StringLib {
    ...
    setLength(pointer, length) {
        this.memory.set(pointer + 1, length);
    }
    charArrayPointer(pointer) {
        return this.memory.get(pointer + 2);
    }
    charAt(pointer, index) {
