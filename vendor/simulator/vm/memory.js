...
export class VmMemory extends RAM {
    ...
    baseSegment(segment, offset) {
        if (this.strict && (offset < 0 || offset > 32767))
            return Err(new Error(`Illegal offset value ${offset} (must be between 0 and 32767)`));
        switch (segment) {
            case "argument":
            ...
            case "pointer":
                if (this.strict && offset > 1)
                    throw new Error(`pointer out of bounds access (pointer can be 0 for this, 1 for that, but got ${offset}`);
            ...
            case "temp":
                if (this.strict && offset > 7)
                    return Err(new Error(`Temp out of bounds access (temp can be 0 to 7, but got ${offset}`));
                return Ok(5 + offset);
            case "that":
    ...
    getSegment(segment, offset) {
        if (segment === "constant") {
            if (this.strict && (offset < 0 || offset > 32767))
                throw new Error(`Illegal offset value ${offset} (must be between 0 and 32767)`);
            return offset;
        }
    ...
    setSegment(segment, offset, value) {
        const base = this.baseSegment(segment, offset);
        if (isErr(base)) {
            throw Err(base);
        }
    ...
    getFrame(base, // The address of the frame, the RET address
    argN, // The number of arguments to this frame
    localN, // The number of locals in this frame
    thisN, // The number of items in `this`
    thatN, // the number of items in `that`
    nextFrame) {
        const arg = base - argN;
        const lcl = base + 5;
