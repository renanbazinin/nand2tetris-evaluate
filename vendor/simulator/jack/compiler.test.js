...
describe("compiler", () => {
    ...
    it("compiles function", () => {
        const func = parse(`function void main() {
        var int a;
        let a = 4;
        return;
    ...
    it("compiles if-else", () => {
        const statement = parse(`if (condition) {
      let x = 4;
    } else {
      let x = 5;
    }`, "Statement").statement;
    ...
    it.each(Object.keys(Programs))("%s", (program) => {
        const compiled = compile(Object.fromEntries(Object.entries(Programs[program]).map(([name, file]) => [
            name,
            file.jack,
        ])));
