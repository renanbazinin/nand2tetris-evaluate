...
import { ChipTest, TestEvalInstruction, TestTickInstruction, TestTockInstruction, } from "./chiptst.js";
import { TestCompoundInstruction, TestOutputInstruction, TestSetInstruction, } from "./instruction.js";
describe("Chip Test", () => {
    describe("Builtins", () => {
    ...
    describe("Full tests", () => {
        ...
        it("tick tocks a clock with a repeat", async () => {
            ...
            test.outputList([{ id: "time", style: "S", len: 4, lpad: 0, rpad: 0 }]);
            await test.run();
            expect(test.log().trim().split("\n")).toEqual(["0+", "1", "1+", "2", "2+", "3", "3+", "4", "4+", "5"].map((i) => `|${i.padEnd(4, " ")}|`));
        });
    });
    it("has a first step", () => {
        const test = new ChipTest(); //.with(new DFF());
...
//# sourceMappingURL=chiptst.test.js.map