...
export function isCommandAsm(command) {
    return (commandASMValues.has(command) ||
        commandASMValues.has(command.replace("M", "A")));
}
...
export const COMMANDS = {
    ...
    getOp(asm) {
        return COMMANDS.asm[asm.replace("M", "A")];
    },
};
...
export function isAssignAsm(assign) {
    return assignAsmValues.has(assign);
}
export const ASSIGN = {
    asm: {
        "": 0x0,
...
export function isJumpAsm(jump) {
    return jumpAsmValues.has(jump);
}
export const JUMP = {
    asm: {
        "": 0b0,
