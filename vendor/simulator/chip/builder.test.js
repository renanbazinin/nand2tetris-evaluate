...
describe("Chip Builder", () => {
    it("builds a chip from a string", async () => {
        const nand = unwrap(await parse(`CHIP Not { IN in; OUT out; PARTS: Nand(a=in, b=in, out=out); }`));
        nand.in().pull(LOW);
        nand.eval();
    ...
    it("builds and evals a chip with subbus components", async () => {
        let foo;
        try {
            foo = unwrap(await parse(`CHIP Foo {
          IN six[3];
          OUT out;
          PARTS: Not16(
            in[0..1] = true,
    ...
    it("looks up unknown chips in fs", async () => {
        const fs = new FileSystem(new ObjectFileSystemAdapter({ "Copy.hdl": COPY_HDL }));
        let foo;
        try {
    ...
    it("returns error for mismatching input width", async () => {
        try {
            const chip = unwrap(HDL.parse(`CHIP Foo {
        IN in[3]; OUT out;
    ...
    it("returns error for wire loop", async () => {
        try {
            const chip = unwrap(HDL.parse(`CHIP Not {
        IN in;
        OUT out;
        PARTS:
    ...
    it("sorts after wiring", async () => {
        try {
            const chip = unwrap(HDL.parse(`CHIP Or { IN a, b; OUT out;
  PARTS:
  Not(in =b , out = net2);
